...
layout(location = 4) out vec4 emissionOut;
layout(location = 5) out vec4 oppositePosOut;
layout(location = 6) out vec4 refractionOut;
...
void main() {    
    ...
    emissionOut = vec4(emissionColor + emissive, 1.0f);
    ...
    oppositePosOut = vec4(oppositePos, 1.0f);

    refractionOut = vec4(1.0f);
}
